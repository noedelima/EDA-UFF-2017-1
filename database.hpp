	...
	public:
		...
		int inserting(node* element){
			...
			/*if(level_left - level_right > 1){
				level_left = (*ptrleft).leftrotate();
			}
			if(level_right - level_left > 1){
				level_right = (*ptrright).rightrotate();
			}*/
			if (level_right > level_left){return level_right;}
			else {return level_left;}
		}
		/*int leftrotate(){
			node* temp3 = (*ptrleft).ptrright;
			int temp3_level;
			if(temp3->level_left > temp3->level_right){temp3_level = temp3->level_left;}
			else{temp3_level = temp3->level_right;}
			node* temp2 = this;
			int temp2_level_left = temp3_level;
			node* temp1 = ptrleft;
			//int temp1_level_right = this->level_right;
			this = temp1;
			ptrright = temp2;
			temp2->ptrleft = temp3;
			ptrtop = temp2->ptrtop;
			temp2->ptrtop = this;
			temp3->ptrtop = temp2;
			temp2->level_left = temp2_level_left;
			if(temp2->level_left > temp2->level_right){level_right = temp2->level_left;}
			else{level_right = temp2->level_right;}
			if(level_right > level_left){return level_right;}
			else{return level_left;}
		}
		int rightrotate(){
			node* temp3 = (*ptrright).ptrleft;
			int temp3_level;
			if(temp3->level_right > temp3->level_left){temp3_level = temp3->level_right;}
			else{temp3_level = temp3->level_left;}
			node* temp2 = this;
			int temp2_level_right = temp3_level;
			node* temp1 = ptrright;
			//int temp1_level_left = this->level_left;
			this = temp1;
			ptrleft = temp2;
			temp2->ptrright = temp3;
			ptrtop = temp2->ptrtop;
			temp2->ptrtop = this;
			temp3->ptrtop = temp2;
			temp2->level_right = temp2_level_right;
			if(temp2->level_right > temp2->level_left){level_left = temp2->level_right;}
			else{level_left = temp2->level_left;}
		...
		void getrange(queue<T>* list, Ref min, Ref max){
			if(this->key>min && this->key<max){
				if(ptrleft){getrange(list, min, max);}
				T* data = new T;
				*data = *this->content;
				(*list).push(data);
				if(ptrright){getrange(list, min, max);}
			}
		}
	};
	node* ptrfather;
